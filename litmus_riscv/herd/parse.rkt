...
(struct litmus-file (name cpu content pre globals insns post allowed) #:transparent)
...
(define (parse-litmus-test s)
  ...
  (match-define (list _ allowed-str)
    (let ([n (string-index s #\newline)])
      (begin0
        (string-split (substring s 0 n))
        (set! s (string-trim (substring s (+ n 1)))))))

  (define allowed (list (string->symbol allowed-str)))
  ;;; (displayln "allowed")
  ;;; (pretty-print allowed)
  ...
  (litmus-file name cpu content init mem procs post allowed))
